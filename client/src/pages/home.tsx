    ...
    <Layout>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary mb-8 text-sm font-medium">
              ...
              Открыта запись на текущий месяц
            </div>
            <h1 className="text-5xl md:text-7xl font-bold mb-6 tracking-tight leading-tight">
              Место, где твой звук <br/>
              <span className="text-gradient">обретает душу</span>
            </h1>
            <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-10 leading-relaxed">
              Премиальное аналоговое оборудование и цифровая точность. 
              Творческое пространство, созданное для воплощения твоих идей.
            </p>
      ...
      <section className="py-24 border-y border-white/5 bg-white/[0.02]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { icon: Disc, title: "Топовое оборудование", desc: "Предусилители и приборы от Neve, SSL и Universal Audio." },
            { icon: Headphones, title: "Акустический комфорт", desc: "Подготовленные комнаты для идеального контроля и записи." },
            { icon: Play, title: "Опытные инженеры", desc: "Команда профессионалов, преданная вашему творчеству." },
          ].map((item, i) => (
            <motion.div 
              key={i}
              initial={{ opacity: 0, y: 20 }}
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between mb-12">
          <div>
            ...
            <p className="text-muted-foreground text-lg">Все, что нужно: от записи до финального мастера.</p>
          </div>
            ...
            {services?.slice(0, 3).map((service) => (
              <div key={service.id} className="glass-panel p-6 rounded-2xl flex flex-col hover-glow transition-all">
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/10">
                  <span className="text-sm text-muted-foreground">{service.durationMins} мин</span>
      ...
      <section className="py-24 bg-white/[0.02] border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center">Отзывы артистов</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {!reviews?.length && (
                <div className="col-span-full text-center text-muted-foreground py-12">
                  Пока нет отзывов. Станьте первым, кто запишется у нас!
                </div>
              )}
            </div>
          )}
